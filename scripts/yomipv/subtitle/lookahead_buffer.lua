...
local function parse_srt(raw)
	...
	msg.info(string.format("Lookahead: Parsed %d subtitle entries", #entries))
	return entries
end

function LookaheadBuffer.reset()
	LookaheadBuffer._entries = {}
	LookaheadBuffer._ready = false
end

function LookaheadBuffer.load()
	LookaheadBuffer.reset()

	local path = mp.get_property("path")
	if not path then
		msg.warn("Lookahead: No file path available")
		return
	end
	...
	if sub_file and sub_file ~= "" then
		msg.info("Lookahead: Using external subtitle file: " .. sub_file)
		LookaheadBuffer._extract_from_file(sub_file)
		return
	end
	...
	if not track_id then
		msg.info("Lookahead: No active subtitle track found")
		return
	end

	msg.info(string.format("Lookahead: Extracting internal track (ff-index %d) via ffmpeg", track_id))
	LookaheadBuffer._extract_from_video(path, track_id)
end

-- Extract text from standalone subtitle files
function LookaheadBuffer._extract_from_file(file_path)
	mp.command_native_async({
		...
		if not success or result.status ~= 0 or not result.stdout or result.stdout == "" then
			...
			msg.info("Lookahead: ffmpeg failed on external file, trying plain read")
			local f = io.open(file_path, "r")
			if f then
				local raw = f:read("*a")
				f:close()
				LookaheadBuffer._entries = parse_srt(raw)
				LookaheadBuffer._ready = #LookaheadBuffer._entries > 0
			end
			return
		end

		LookaheadBuffer._entries = parse_srt(result.stdout)
		LookaheadBuffer._ready = #LookaheadBuffer._entries > 0
	end)
end

-- Extract subtitles from video container using ffmpeg
function LookaheadBuffer._extract_from_video(video_path, ff_index)
	...
	mp.command_native_async({
		...
		if not success or result.status ~= 0 or not result.stdout or result.stdout == "" then
			msg.warn("Lookahead: ffmpeg extraction failed for internal track")
			return
		end
...
function LookaheadBuffer.get_next_lines(current_time, current_text, count)
	if not LookaheadBuffer._ready then
		return {}
	end
	...
	for _, entry in ipairs(LookaheadBuffer._entries) do
